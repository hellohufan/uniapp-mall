			...
			<!-- 直播 -->
				<view class="live-wrapper-a">
					<navigator class="live-item-a" v-for="(item,index) in liveList" :key="index" :url="'plugin-private://wx2b03c6e691cd7370/pages/live-player-plugin?room_id=' + item.room_id"
					 hover-class="none" :style="[{'background':bg},{'box-shadow':`0px 1px 20rpx ${boxShadow}`}]">
						<view class="img-box">
	...
	export default {
		...
		data() {
			return {
				...
				mbConfig:10,
				liveList:[],
				bg: this.dataConfig.listBg.color[0].item,
				boxShadow: this.dataConfig.shadow.color[0].item,
